        ...
        function obj      = createGrid(obj)
            ...
            if strcmp(obj.method, 'NI')
                ...
                if obj.GridData.N == 0
                    ...
                    %weight = obj.NodesWeights2('Uniform', obj.GridData.N(n)+1);
                    %s = weight(:,2)';
                    s = linspace(obj.Distribution.minv, obj.Distribution.maxv, obj.GridData.N+1);
                    
%                     obj.P = diag(ones(obj.GridData.N + 1, 1), 0);
%                     obj.P(1:8,1:8)=diag([1498139/5080320, 1107307/725760, 20761/80640, 1304999/725760, 299527/725760, 103097/80640, 670091/725760, 5127739/5080320]);
%                     obj.P(obj.GridData.N+1-7:obj.GridData.N+1,obj.GridData.N+1-7:obj.GridData.N+1)=rot90(diag([1498139/5080320, 1107307/725760, 20761/80640, 1304999/725760, 299527/725760, 103097/80640, 670091/725760, 5127739/5080320]), 2);
%                     obj.P = 2/(obj.GridData.N)*obj.P;
                    
                end
                
                obj.GridData.grid = s;
                obj.GridData.d = (obj.Distribution.maxv-obj.Distribution.minv)/(obj.GridData.N);
                
            elseif strcmp(obj.method, 'PC')
                
                obj.MaxPol = max(5, obj.GridData.N);
...
%                 obj.IPfact = zeros(obj.GridData.N + 1, obj.MaxPol + 1);
%                 
%                 for l = 1:obj.MaxPol + 1
%                     for m = 0:obj.GridData.N
% 
...
%                 end
                ...
                eval(['load(''PC_BasisFunctions_All''', ',''IPfact', num2str(obj.GridData.N),''')'])
                
                obj.IPfact = eval(['IPfact', num2str(obj.GridData.N),'']);
                
                obj.GridData.grid = 0:obj.GridData.N;
        ...
        function moment   = computeMoment(obj, u_ns, m)
            ...
            if strcmp(obj.method, 'NI')
                ...
                elseif strcmp(obj.Distribution.name, 'Uniform')
                    ...
                    pdf = ones(obj.GridData.N + 1, 1) * 1/2; %ones(obj.N(1) + 1, 1) * 1 / (obj.N(1) + 1);
                    
                end
                
                momentgrid = zeros(size(u_ns(:,:,1), 1), size(u_ns(:,:,1), 2), obj.GridData.N + 1);
                
                for k = 1:obj.GridData.N + 1
        ...
        function [mean_t, var_t, obj] = functionals(obj, grid_obj, u_ns)
            ...
            mean_cmp_normed     = sum(abs(mean_analytical - obj.statistics.mean), 2)         .* grid_obj.d(1);
            variance_cmp_normed = sum(abs(variance_analytical - obj.statistics.variance), 2) .* grid_obj.d(1);
            
            mean_t              = sum(mean_cmp_normed)     * grid_obj.d(end);
            var_t               = sum(variance_cmp_normed) * grid_obj.d(end);
            ...
            if strcmp(polynomial, 'Uniform')
                ...
                elseif M == 24
                    ...
                    w = [0.0123412297999871995468	-0.99518721999702136018;
                        ...
                        0.0442774388174198061686	0.9382745520027327585236;
                        0.0285313886289336631813	0.9747285559713094981984;
                        0.0123412297999871995468    0.99518721999702136018];
                elseif M == 25
