classdef sbp_obj
    ...
    properties
        ...
        diss; % Dissipation magnitude
        
    end
    methods
        function obj = createDissipationOperator(obj, grid_obj)
            ...
            if obj.acc==2
                % Second order global artificial dissipation operators
                DD = diag(-1*ones(N+1,1)) + diag(ones(N,1),1);
                obj.DI = DD'*B*DD;
                obj.DI(end-1:end,:) = fliplr(flipud(obj.DI(1:2,:)));
                obj.DI = sparse(obj.diss*obj.DI);

            elseif obj.acc==4
                % Third order global artificial dissipation
                DD = diag(-2*ones(N+1,1)) + diag(ones(N,1),1) + diag(ones(N,1),-1);
                DD(1,1)=1; DD(1,2)=-2; DD(1,3)=1;
                obj.DI = DD'*B*DD;
                obj.DI(end-3:end,:) = fliplr(flipud(obj.DI(1:4,:)));
                obj.DI = sparse(obj.diss * obj.DI);

            elseif obj.acc==6
                % Fourth order global artificial dissipation
                DD = diag(-3*ones(N+1,1)) + diag(ones(N,1),1) + diag(3*ones(N,1),-1) + diag(-ones(N-1,1),-2);
                DD(1,1)=-1; DD(1,2)=3; DD(1,3)=-3; DD(1,4)=1;
                DD(2,1)=-1; DD(2,2)=3; DD(2,3)=-3; DD(2,4)=1;
                obj.DI = DD'*B*DD;
                obj.DI(end-5:end,:) = fliplr(flipud(obj.DI(1:6,:)));
                obj.DI = sparse(obj.diss*obj.DI);

            elseif obj.acc==8
                % Fifth order global artificial dissipation
                DD = diag(6*ones(N+1,1)) + diag(-4*ones(N,1),1) + diag(-4*ones(N,1),-1) + diag(ones(N-1,1),2) + diag(ones(N-1,1),-2);
                DD(1,1)=1; DD(1,2)=-4; DD(1,3)=6; DD(1,4)=-4; DD(1,5)=1;
                DD(2,1)=1; DD(2,2)=-4; DD(2,3)=6; DD(2,4)=-4; DD(2,5)=1;
                DD(3,1)=1; DD(3,2)=-4; DD(3,3)=6; DD(3,4)=-4; DD(3,5)=1;
                obj.DI = DD'*B*DD;
                obj.DI(end-7:end,:) = fliplr(flipud(obj.DI(1:8,:)));
                obj.DI = sparse(obj.diss*obj.DI);
