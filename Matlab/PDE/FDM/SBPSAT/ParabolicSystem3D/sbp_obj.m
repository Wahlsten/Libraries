                ...
                if N == 0
                    ...
                    if obj.acc==2
                        ...
                        if strcmp(type,'wide')
                            ...
                            obj.D2{k} = ((diag(ones(N,1),1)+diag(ones(N,1),-1)-2*diag(ones(N+1,1),0)));
                            obj.D2{k}(1,1)=1;obj.D2{k}(1,2)=-2;obj.D2{k}(1,3)=1;
                            obj.D2{k}(N+1,N+1-2)=1;obj.D2{k}(N+1,N)=-2;obj.D2{k}(N+1,N+1)=1;
                            obj.D2{k} = sparse(obj.D2{k}/dx^2);
                            
                            obj.BS{k} = zeros(N+1);
                            obj.BS{k}(1,1:3) = -[-3/2, 2, -1/2];
                            obj.BS{k}(N+1,N-1:N+1)=fliplr(-[-3/2, 2, -1/2]);
                            obj.BS{k} = sparse(obj.BS{k}/dx);
                        end
                    end
                    ...
                    if obj.acc==4
                        ...
                        if strcmp(type,'wide')
                            ...
                            obj.D2{k} = spdiags(dia,[-2:2],N+1,N+1);
                            obj.D2{k}(1:4,1:6) = [2    -5      4      -1     0     0; ...
                                1    -2      1      0      0     0; ...
                                -4/43 59/43 -110/43 59/43 -4/43   0; ...
                                -1/49   0    59/49 -118/49 64/49 -4/49];
                            obj.D2{k}(N+1-3:N+1,N+1-5:N+1) = flipud(fliplr(obj.D2(1:4,1:6)));
                            obj.D2{k} = sparse(obj.D2/(dx^2));
                            
                            obj.BS{k} = zeros(N+1);
                            obj.BS{k}(1,1:4) = [11/6,-3,3/2,-1/3];
                            obj.BS{k}(N+1,N+1:-1:N-2) = obj.BS(1,1:4);
                            obj.BS{k} = sparse(obj.BS/dx);
                        end
                    end
                    ...
                    if obj.acc==6
                        ...
                        if strcmp(type,'wide')
                            ...
                            obj.D2{k} = (2*diag(ones(N+1-3,1),3)-27*diag(ones(N+1-2,1),2)+270*diag(ones(N,1),1)+270*diag(ones(N,1),-1)-27*diag(ones(N+1-2,1),-2)+2*diag(ones(N+1-3,1),-3)-490*diag(ones(N+1,1),0))/180;
                            obj.D2{k}(1:6,1:9)=[114170/40947, -438107/54596, 336409/40947, -276997/81894, 3747/13649, 21035/163788, 0, 0, 0;6173/5860, -2066/879, 3283/1758, -303/293, 2111/3516, -601/4395, 0, 0, 0;-52391/81330, 134603/32532, -21982/2711, 112915/16266, -46969/16266, 30409/54220, 0, 0, 0;68603/321540, -12423/10718, 112915/32154, -75934/16077, 53369/21436, -54899/160770, 48/5359, 0, 0;-7053/39385, 86551/94524, -46969/23631, 53369/15754, -87904/23631, 820271/472620, -1296/7877, 96/7877, 0;21035/525612, -24641/131403, 30409/87602, -54899/131403, 820271/525612, -117600/43801, 64800/43801, -6480/43801, 480/43801];
                            obj.D2{k}(N+1-5:N+1,N+1-8:N+1)=flipud( fliplr(obj.D2(1:6,1:9) ) );
                            obj.D2{k} = sparse(obj.D2/dx^2);
                            
                            obj.BS{k} = zeros(N+1);
                            obj.BS{k}(1,1:5) = -[-25/12, 4, -3, 4/3, -1/4];
                            obj.BS{k}(N+1,N-3:N+1) = fliplr(-[-25/12, 4, -3, 4/3, -1/4]);
                            obj.BS{k} = sparse(obj.BS/dx);
                        end
                    ...
                    if obj.acc==8
                        ...
                        obj.D1{k}(N+1-7:N+1,N+1-11:N+1)=flipud( fliplr(-obj.D1{k}(1:8,1:12)));
                        obj.D1{k} = sparse(obj.D1{k}/dx);
